...
export const addNotebook = async (name) => {
  try {
    const response = await axios.post('https://ismailtag2.sandsculptor.keiken-digital.io:8080/api/notebooks', {
      title: name
    });
...
export const getNotebooks = async () => {
  try {
    const response = await axios.get('https://ismailtag2.sandsculptor.keiken-digital.io:8080/api/notebooks');
...
export const deleteNotebook = async (id) => {
  try {
    await axios.delete(`https://ismailtag2.sandsculptor.keiken-digital.io:8080/api/notebooks/${id}`);
  } catch (err) {
...
export const addNote = async (notebookId, title, content) => {
  try {
    const response = await axios.post(`https://ismailtag2.sandsculptor.keiken-digital.io:8080/api/notebooks/${notebookId}/notes`, {
      title: title,
      content: content
    });
...
export const getNotes = async (notebookId) => {
  try {
    const response = await axios.get(`https://ismailtag2.sandsculptor.keiken-digital.io:8080/api/notebooks/${notebookId}/notes`);
...
export const getNotesSorted = async (notebookId) => {
  try {
    const response = await axios.get(`https://ismailtag2.sandsculptor.keiken-digital.io:8080/api/notebooks/${notebookId}/notes?sort=true`);
    return response.data; // Assumes notes are under the 'notes' field in response
  } catch (err) {
    console.error("An API error has occurred while fetching notes:", err);
    return []; // Return an empty array as a fallback
...
export const updateNote = async (id, title, content) => {
  try {
    const response = await axios.put(`https://ismailtag2.sandsculptor.keiken-digital.io:8080/api/notes/${id}`, {
      title: title,
      content: content
    });
...
export const deleteNote = async (id) => {
  try {
    await axios.delete(`https://ismailtag2.sandsculptor.keiken-digital.io:8080/api/notes/${id}`);
  } catch (err) {
